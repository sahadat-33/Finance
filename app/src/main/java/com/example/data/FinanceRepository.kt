...
class FinanceRepository(private val context: Context) {
    ...
    suspend fun insertSavingsVault(vault: SavingsVault) = withContext(Dispatchers.IO) {
        ...
        if (existing != null) {
            ...
            val maxOrder = (dao.getAllSavingsVaults().maxOfOrNull { it.sortOrder } ?: 0) + 1
            dao.insertSavingsVault(vault.copy(updatedAt = System.currentTimeMillis(), sortOrder = maxOrder))
        }
        triggerImmediateSync()
    }
    ...
    suspend fun insertCategory(category: Category) = withContext(Dispatchers.IO) {
        val maxOrder = (dao.getAllCategories().maxOfOrNull { it.sortOrder } ?: 0) + 1
        dao.insertCategory(category.copy(sortOrder = maxOrder))
        triggerImmediateSync()
    }

    suspend fun updateCategoriesOrder(reorderedList: List<Category>) = withContext(Dispatchers.IO) {
        val now = System.currentTimeMillis()
        val updated = reorderedList.mapIndexed { index, cat ->
            cat.copy(sortOrder = index, updatedAt = now)
        }
        dao.updateCategories(updated)
        triggerImmediateSync()
    }

    suspend fun updateSavingsVaultsOrder(reorderedList: List<SavingsVault>) = withContext(Dispatchers.IO) {
        val now = System.currentTimeMillis()
        val updated = reorderedList.mapIndexed { index, vault ->
            vault.copy(sortOrder = index, updatedAt = now)
        }
        dao.updateSavingsVaults(updated)
        triggerImmediateSync()
    }
